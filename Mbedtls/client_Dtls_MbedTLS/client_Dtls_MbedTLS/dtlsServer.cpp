...
//#endif
...
#endif
    //config the ssl handshake proccess:
    uint32_t min_timeout = 0; // 5 seconds
    uint32_t max_timeout = 30000; // 30 seconds
    mbedtls_ssl_conf_handshake_timeout(&conf, min_timeout, max_timeout);
    /*
     * 1. Seed the RNG
     */
     ...
     */
    ret = mbedtls_x509_crt_parse_file(&srvcert, SERVER_CERT_PATH);
    if (ret != 0) {
        printf(" failed\n  !  mbedtls_x509_crt_parse returned %d\n\n", ret);
        goto exit;
    }

    ret = mbedtls_x509_crt_parse_file(&srvcert, CA_CERT_PATH);
    if (ret != 0) {
        printf(" failed\n  !  mbedtls_x509_crt_parse returned %d\n\n", ret);
        goto exit;
    }

    ret = mbedtls_pk_parse_keyfile(&pkey, SERVER_KEY_PATH, NULL, mbedtls_ctr_drbg_random, &ctr_drbg);
