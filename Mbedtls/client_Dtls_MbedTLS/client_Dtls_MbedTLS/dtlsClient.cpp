...
#define CLIENT_CERT_PATH    "C:\\Users\\israe\\OneDrive - ort braude college of engineering\\rsa_encryption_new\\rsa_encryption_new\\client-signed-cert.pem"
#define CLIENT_KEY_PATH     "C:\\Users\\israe\\OneDrive - ort braude college of engineering\\rsa_encryption_new\\rsa_encryption_new\\client-key.pem"
...
int main2(int argc, char* argv[])
{
    ...
    if ((ret = mbedtls_x509_crt_parse_file(&cacert, CA_CERT_PATH)) != 0) {
    ...
    mbedtls_printf("  . Loading the client certificate...");
    fflush(stdout);

    if ((ret = mbedtls_x509_crt_parse_file(&client_cert, CLIENT_CERT_PATH)) != 0) {
        mbedtls_printf(" failed\n  !  mbedtls_x509_crt_parse_file returned -0x%x\n\n",
            (unsigned int)-ret);
        goto exit;
    }
    ...
    if ((ret = mbedtls_pk_parse_keyfile(&client_key, CLIENT_KEY_PATH, NULL, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0) {
